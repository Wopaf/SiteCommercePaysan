...
function renderOrders() {
    ...
    const unprocessedOrders = DATA.orders.filter(o => !o.processed);
    const processedOrders = DATA.orders.filter(o => o.processed);
    ...
    const sortOrders = (orders) => {
        return [...orders].sort((a, b) => {
            ...
            return orderFilters.sortOrder === 'desc' ? valB - valA : valA - valB;
        });
    };
    
    const sortedUnprocessed = sortOrders(unprocessedOrders);
    const sortedProcessed = sortOrders(processedOrders);
    
    container.innerHTML = `
        <div class="filters-bar" style="background:white;padding:1rem;border-radius:10px;margin-bottom:1rem;display:flex;gap:1rem;flex-wrap:wrap;">
            <button class="btn-secondary btn-sm" onclick="applySortOrders('date')">
                📅 Date ${orderFilters.sortBy === 'date' ? (orderFilters.sortOrder === 'desc' ? '↓' : '↑') : ''}
            </button>
            <button class="btn-secondary btn-sm" onclick="applySortOrders('articles')">
                📦 Articles ${orderFilters.sortBy === 'articles' ? (orderFilters.sortOrder === 'desc' ? '↓' : '↑') : ''}
            </button>
            <button class="btn-secondary btn-sm" onclick="applySortOrders('total')">
                💰 Total ${orderFilters.sortBy === 'total' ? (orderFilters.sortOrder === 'desc' ? '↓' : '↑') : ''}
            </button>
            <button class="btn-secondary btn-sm" onclick="toggleSortOrder()">
                🔄 Inverser
            </button>
        </div>
        
        ${sortedUnprocessed.length > 0 ? `
            <div style="margin-bottom:2rem;">
                <h3 style="color:#ff8f3c;margin-bottom:1rem;">⚠️ Non Traitées (${sortedUnprocessed.length})</h3>
                <table style="width:100%;background:white;border-radius:15px;overflow:hidden;border:3px solid #ff8f3c;">
                    <thead style="background:#ff8f3c;color:white;">
                        <tr>
                    ...
                    <tbody>
                        ${sortedUnprocessed.map(order => `
                            <tr style="border-bottom:1px solid #eee;cursor:pointer;background:#fff3e0;" onclick="showOrderDetails('${order.id}')">
        ...
        <h3 style="color:#4a7c4e;margin-bottom:1rem;">✅ Traitées (${sortedProcessed.length})</h3>
...
function showOrderDetails(orderId) {
    ...
    document.getElementById('orderDetailsContent').innerHTML = `
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;">
            <button class="btn-${order.processed ? 'secondary' : 'primary'}" onclick="toggleOrderStatus('${order.id}')">
                ${order.processed ? '✅ Traitée' : '⚠️ Marquer traitée'}
            </button>
        </div>
        <p><strong>Statut:</strong> <span style="color:${order.processed ? '#4a7c4e' : '#ff8f3c'};font-weight:600;">${order.processed ? '✅ Traitée' : '⚠️ En attente'}</span></p>
...
// Fermer modals admin sur overlay
setTimeout(() => {
